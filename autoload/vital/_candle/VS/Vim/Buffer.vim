...
execute join(['function! vital#_candle#VS#Vim#Buffer#import() abort', printf("return map({'get_line_count': '', 'do': '', 'create': '', 'load': ''}, \"vital#_candle#function('<SNR>%s_' . v:key)\")", s:_SID()), 'endfunction'], "\n")
delfunction s:_SID
" ___vital___
let s:Do = { -> {} }
...
if exists('*bufload')
  function! s:load(bufnr_or_path) abort
    silent call bufload(bufnr(a:bufnr_or_path, v:true))
  endfunction
else
  function! s:load(bufnr_or_path) abort
